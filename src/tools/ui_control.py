...
class UIControlTool(AlfredTool):
    ...
    description = (
        "Work inside a Windows app through its accessibility tree - "
        "precise, by name, no guessing at pixels. The usual sequence "
        "after open_app is wait_ready, then search, then open_item; "
        "drop to tree/click/type only when those cannot say what you "
        "mean. "
        "WHOLE JOBS: search (type into the app's own search box and "
        "submit); open_item (open a result, row, tile, save or "
        "instance by name); links (the things ON a page, with the "
        "site's navigation stripped out); clear_popups (close what is "
        "in the way, and report anything that needs the user). "
        "SINGLE STEPS: windows, focus, tree, find, click, "
